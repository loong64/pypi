...
# https://github.com/h5py/h5py/blob/master/ci/get_hdf5_if_needed.sh
PROJECT_ROOT="$(pwd)"
...
if [ -z ${HDF5_DIR+x} ]; then
    ...
    if [ -f $HDF5_DIR/lib/$lib_name ]; then
        ...
        curl -fsSL -o bin/config.sub 'https://git.savannah.gnu.org/gitweb/?p=config.git;a=blob_plain;f=config.sub;hb=HEAD'
        curl -fsSL -o bin/config.guess 'https://git.savannah.gnu.org/gitweb/?p=config.git;a=blob_plain;f=config.guess;hb=HEAD'
